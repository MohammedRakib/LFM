...
def val(epoch, val_loader, model, logger):
    ...
    with torch.no_grad():
        for step, (spectrogram, image, y) in enumerate(tqdm(val_loader)):
            ...
            y_one_hot = F.one_hot(y, num_classes=cfg["setting"]["num_class"]).float()  # Convert y to one-hot encoding
            one_hot_label = one_hot_label + y_one_hot.tolist()
            image = image.cuda()
            y = y.cuda()
            # spectrogram = spectrogram.unsqueeze(1).float().cuda()
            spectrogram = spectrogram.float().cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config',type=str, default='./data/avmnist.json')
    ...
    #     save_path ="/home/rakib/LFM/modelWeight/"
    #     os.makedirs(save_path, exist_ok=True)  # Create directory if it does not exist
    #     torch.save(model.state_dict(), os.path.join(save_path, f'AVMNIST_{epoch}_best_model.pth'))
    ...
    torch.save(best_model, os.path.join(save_path, f'AVMNIST_best_model.pth'))

    # Print the best accuracy
    logger.info(f"Best Accuracy: {best_acc:.4f} achieved at Epoch {best_epoch}")
    print(f"Best Accuracy: {best_acc:.4f} achieved at Epoch {best_epoch}")
